$(function () {
  $(document).on('click', 'td[data-role=deletePayment]', function () {
    const id = ($(this).data('id'));
    const payment = $(this).closest('tr').children('td:first').text();

    bootbox.confirm({
      message: "Do you really want to delete payment method?",
      centerVertical: true,
      backdrop: true,
      buttons: {
        confirm: {
          ...
          $.ajax({
            ...
            success: function (response) {
              ...
              if (response) {
                ...
                bootbox.alert({
                  message: '<i class="bi bi-exclamation-square pr-2"></i>Payments in this category have been moved to the "Another"!',
                  centerVertical: true,
                  backdrop: true,
                });
